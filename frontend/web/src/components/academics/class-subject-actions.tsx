...
export function ClassSubjectActions({ classGroup, subjects, academicSessionId }: { classGroup: ClassGroup; subjects: Subject[]; academicSessionId: string }) {
  ...
  }, [open, groupOpen, classGroup.id, academicSessionId, loadGroups]);
  ...
  function beginGroup(group?: Group) { setEditing(group ?? null); setGroupName(group?.displayName ?? ""); setGroupSelected(group?.members.map(x => x.classSubjectId) ?? []); setGroupError(null); setGroupOpen(true); void loadGroups(); }
  async function resetGroups() {
    ...
    const result = await response.json().catch(() => ({}));
    if (!response.ok) { setGroupError(result.error ?? "Unable to reset parallel groups."); return; }
    setGroups([]);
    setGroupSelected([]);
    setEditing(null);
  }
    ...
    <Dialog open={groupOpen} onOpenChange={setGroupOpen}>
      <DialogContent>
        ...
        <div className="border-t pt-3">
          <div className="flex items-center justify-between"><h4 className="font-semibold">Configured groups</h4><Button size="sm" variant="outline" onClick={resetGroups} disabled={groupLoading || groups.length === 0}>Reset Parallel Groups</Button></div>
          {groups.length === 0 && !groupLoading ? <p className="mt-2 text-sm text-muted-foreground">No parallel groups configured for this class and session.</p> : groups.map(group => <div key={group.id} className="mt-2 rounded-md border p-3 text-sm"><div className="font-medium">{group.displayName || group.members.map(x => x.subjectName).join(" / ")}</div><div className="text-muted-foreground">{group.sharedPeriodsPerWeek} shared periods/week · {group.members.map(x => `${x.subjectName} — ${x.teacherName || "Teacher not assigned"}`).join("; ")}</div><div className="mt-2 flex gap-2"><Button size="sm" variant="outline" onClick={() => beginGroup(group)}>Edit</Button><Button size="sm" variant="outline" onClick={() => removeGroup(group.id)}>Delete</Button></div></div>)}
        </div>
      </DialogContent>
    </Dialog>
  </>;
